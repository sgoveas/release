...
# For OCP 5.1+, CAPI may be GA and not require CustomNoUpgrade
# Update the script accordingly if CAPI is not GA in 5.1
if [[ "${ocp_major}" -gt 5 ]] || [[ "${ocp_major}" -eq 5 && "${ocp_minor}" -ge 1 ]]; then
  echo "[INFO] OCP version ${ocp_version} is 5.1+, CAPI may be GA - skipping feature gate configuration"
else
  # Verify TechPreviewNoUpgrade feature set is enabled
  current_featureset=$(oc get featuregate cluster -o jsonpath='{.spec.featureSet}')
  capi_enabled=$(oc get featuregate cluster -o jsonpath='{.status.featureGates[*].enabled[?(@.name=="ClusterAPIMachineManagement")].name}' 2>/dev/null | grep -o "ClusterAPIMachineManagement" || true)
  if [ "$current_featureset" = "TechPreviewNoUpgrade" ] && [ -n "$capi_enabled" ]; then
    echo "[INFO] TechPreviewNoUpgrade feature set is enabled"
  else
    echo "ERROR: TechPreviewNoUpgrade feature set is not enabled. Exiting"
    exit 1
  fi
fi
